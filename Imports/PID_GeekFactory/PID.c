...
pid_t pid_create(pid_t pid, float* in, float* out, float* set, float kp, float ki, float kd)
{
	...
	pid->lasttime = Clock_getTicks() - pid->sampletime;
...
bool pid_need_compute(pid_t pid)
{
	...
	return(Clock_getTicks() - pid->lasttime >= pid->sampletime) ? true : false;
}
...
void pid_compute(pid_t pid)
{
	...
	if (!pid->automode)
		System_abort("Error in PID.c ln53-ish");
